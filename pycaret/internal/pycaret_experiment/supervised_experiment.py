...
class _SupervisedExperiment(_TabularExperiment):
    ...
    def compare_models(
        ...
        for i, model in enumerate(model_library):
            ...
            st.toast(f"Finished training :blue[{model_name}]", icon="✅")
    ...
    def tune_model(
        ...
        **kwargs:
        ...
        st.toast(f"Fine Tuning :blue[{estimator_name}]", icon="⚙️")
        with st.spinner(
            f"Fine tuning :blue[{estimator_name}]{'' if 'MLP' not in estimator_name else ', this may take a while...'}"
        ):
            ...
            if custom_grid is not None:
                if not isinstance(custom_grid, dict):
                    raise TypeError(
                        f"custom_grid must be a dict, got {type(custom_grid)}."
                    )
            ...
            with estimator_pipeline(self.pipeline, model) as pipeline_with_model:
                ...
                # special case to prevent running out of memory
                if isinstance(
                    pipeline_with_model.steps[-1][1], GaussianProcessClassifier
                ):
                    n_jobs = 1
                ...
                if search_library == "optuna":
                    ...
                    with (
                        ...
                        if search_algorithm == "grid":
                            self.logger.info(
                                "Initializing tune_sklearn.TuneGridSearchCV"
                            )
            ...
            st.toast(f"Fine tuned :green[{estimator_name}] model", icon="✅")
